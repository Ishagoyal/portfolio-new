...
const CaseStudyButton = ({ active, controls, onClick }: { active: boolean; controls: string; onClick: () => void }) => <button className="project-link case-study-button" type="button" aria-expanded={active} aria-controls={controls} onClick={onClick}>{active ? "Close case study" : "Read case study"} {active ? <ArrowUp size={16} /> : <ArrowDown size={16} />}</button>;

const CookBridgePanel = () => <section id="cookbridge-case-study" className="inline-case-study" aria-labelledby="cookbridge-case-heading">
  <div className="inline-case-study__intro"><span id="cookbridge-case-heading">Case study / 01</span><p>Evidence, decisions and iteration from a household meal-decision prototype.</p></div>
...
const FashionPanel = ({ onWatchDemo }: { onWatchDemo: (opener: HTMLElement) => void }) => <section id="fashion-case-study" className="inline-case-study" aria-labelledby="fashion-case-heading">
  <div className="inline-case-study__intro"><span id="fashion-case-heading">Case study / 02</span><p>Evidence and product choices behind a wardrobe-grounded AI prototype.</p></div>
  <div className="case-moments">
...
const Portfolio = () => {
  ...
  return <div className={`site-shell${dark ? " dark" : ""}`}>
    ...
    <main>
      ...
      <section className="profile section-wrap"><SectionTitle number="01">Profile</SectionTitle><div className="profile-copy"><p><strong>I turn early user evidence into focused product decisions and testable prototypes.</strong> The selected work below shows how that judgment holds up in practice.</p></div></section>
      <section className="journey-section section-wrap" id="case-studies"><SectionTitle number="02">Selected work</SectionTitle><div className="project-showcase">
        <article className={`project-feature project-teaser${cookbridgeOpen ? " project-teaser--expanded" : ""}`}><div className="project-visual"><img src="/images/cookbridge-project.jpg" alt="Indian home-cooked meal planning with a phone assistant" /><span>01</span></div><div className="project-story"><p className="project-meta"><span>Selected work</span><span>01</span></p><h3>CookBridge</h3><p className="project-question">Taking “Aaj khane mein kya banega?” out of the household’s head.</p><p className="project-proof">7/10 households validated the problem · Built & testing the prototype</p><Tags items={["0→1 Product", "User Research", "AI", "Prototype"]} /><div className="project-actions"><a className="prototype-link prototype-link--teaser" href="https://cookbridge-whats-for-tea.lovable.app" target="_blank" rel="noreferrer">Try prototype <ArrowUpRight size={16} /></a><CaseStudyButton active={cookbridgeOpen} controls="cookbridge-case-study" onClick={() => setCookbridgeOpen((open) => !open)} /></div></div></article>
        {cookbridgeOpen && <CookBridgePanel />}
        <article className={`project-feature project-teaser project-teaser--fashion${fashionOpen ? " project-teaser--expanded" : ""}`}><button className="project-visual project-visual--playable" type="button" onClick={(event) => openDemo(event.currentTarget)} aria-label="Watch AI Fashion Stylist working demo"><img src="/images/ai-fashion-stylist-project.jpg" alt="Personal wardrobe with three digital outfit suggestions" /><span>02</span><span className="play-affordance"><Play size={20} fill="currentColor" /> Watch demo</span></button><div className="project-story"><p className="project-meta"><span>Selected work</span><span>02</span></p><h3>AI Fashion Stylist</h3><p className="project-question">An AI stylist grounded in clothes you actually own.</p><p className="project-proof">Wardrobe photos → structured context → outfit recommendations</p><Tags items={["AI Vision", "Structured Context", "WhatsApp", "Prototype"]} /><div className="project-actions"><CaseStudyButton active={fashionOpen} controls="fashion-case-study" onClick={() => setFashionOpen((open) => !open)} /></div></div></article>
